...
void
OgreCarRacing::createCamera()
{
	mCamera = mSceneMgr->createCamera("PlayerCam");
	mCamera->setPosition(Ogre::Vector3(0,50,-120));
...
void 
OgreCarRacing::createFrameListener(void)
{
...
void 
OgreCarRacing::createMiniMapCamera(void)
{
	mMiniCamera = mSceneMgr->createCamera("miniCamera");
	//mMiniCamera->pitch(Ogre::Radian(-10.57));
	//mMiniCamera->yaw(Ogre::Radian(-10.57));
	mMiniCamera->setNearClipDistance(5);
	mMiniCamera->setFarClipDistance(1100);
	mMiniCamera->setPosition(Ogre::Vector3(0, 1000, 0));
	mMiniCamera->lookAt(Ogre::Vector3(0, 0, 180));
}

void 
OgreCarRacing::createMiniMapViewports(void)
{
	Ogre::Viewport* hudViewPort = mWindow->addViewport(mMiniCamera, 100, 0, 0.7, 0.3, 0.3);
	hudViewPort->setBackgroundColour(Ogre::ColourValue(0.5, 0.5, 0.5));
	mMiniCamera->setAspectRatio(Ogre::Real(hudViewPort->getActualWidth()) / Ogre::Real(hudViewPort->getActualHeight()));
}
// We will have a single viewport.  If you wanted splitscreen, or a rear-view mirror, then
// you may want multiple viewports.
void 
OgreCarRacing::createViewports(void)
{
...
void
OgreCarRacing::setupResources(void)
{
