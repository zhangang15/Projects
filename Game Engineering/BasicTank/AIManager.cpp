	...
	for (int i = 1; i < MAX_ENEMY_TANK + 1; i++)
	{
		...
		{			
			...
			else
			{
				...
				if (!mWorld ->fireOnList[i] && !(tank->getDamageState()))
				{
					/*Projectile *projectile = new Projectile(mSceneManager);
					projectile -> createProjectile();
					projectile-> mProjectilePosition(tank->getGunWorldPosition());
					mWorld -> projectileList.push_back(projectile);
					mWorld -> projectileDistanceVectorList.push_back(Ogre::Vector3(0,0,500));
					mWorld -> gravityDownList.push_back(false);*/

				/*Projectile *projectile = new Projectile(mSceneManager);
				projectile->createProjectile();
				projectile->mProjectilePosition(tank->getGunWorldPosition());
				mWorld -> projectileList[i] = projectile;*/

				mWorld -> fireOnList[i] = true;
				mWorld -> projectileList[i] -> setProjectileVisibility(true);
				mWorld -> projectileList[i]->mProjectilePosition(tank->getGunWorldPosition());
